...
def setParking(vehicleManual, worldIm):
    global camera_retro, camera_rigth, radar, vehicle
    vehicle = vehicleManual
    radar_bp = worldIm.get_blueprint_library().find('sensor.other.radar')
    ...
    radar = worldIm.spawn_actor(radar_bp, radar_transform, attach_to=vehicle)
    camera_retro = spawn_camera(attach_to=vehicle, world=worldIm, transform=carla.Transform(carla.Location(x=-1.6, z=1.9), carla.Rotation(yaw=180, pitch=-40)))
