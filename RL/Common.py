...
def create_imitation_model(action_space):
    model = ImitationLearningModel(action_space)
    return model

def train_imitation_model(model, observations, actions, epochs=10, batch_size=32):
    model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])
    model.fit(observations, actions, epochs=epochs, batch_size=batch_size)